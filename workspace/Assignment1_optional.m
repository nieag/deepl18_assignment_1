clc, clear all;
rng(400);
%%% Load data for gradient evaluation
...
% [ngrad_b_quick, ngrad_W_quick] = ComputeGradsNum(trainX(:,1), trainY(:, 1), W, b, lambda, 1e-6);
...
GDparams.n_batch=100; % Batch size
GDparams.eta=0.02; % Learning rate
GDparams.n_epochs = 200; % Number of epochs to train
GDparams.decay = 0.9; % Decay rate for the learning rate
...
for i=1:n_epochs
    ...
    if mod(i,10)==0 % Decay the learning rate by factor "decay" every 10 epochs
        eta = eta * decay;    
    end
